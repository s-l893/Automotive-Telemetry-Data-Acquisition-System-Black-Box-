...
static const can_signal_def_t can_signal_table[] = {
    ...
    /* 0x188: byte3[3:0] PRNDS bitmask (0=S,1=P,2=R,4=N,8=D); byte4[4:0] active ratio */
...
{
    (void)dlc; /* not used for bounds-checking yet - table offsets are trusted for now */

    for (uint32_t i = 0; i < CAN_SIGNAL_TABLE_COUNT; i++) {
        const can_signal_def_t *def = &can_signal_table[i];
        if (def->can_id != can_id) {
            continue; /* not a break - one ID can back multiple signals */
        }
