...
void imu_read(void){
	...
	if (!fault_flags.imu_fault && !fault_flags.imu_handshake_fault ){ // allow read to continue if no faults are there
		uint8_t buffer[6];
		imu_accel_read_status = HAL_I2C_Mem_Read(&hi2c1, WRITE_ADDRESS, 59, I2C_MEMADD_SIZE_8BIT, buffer, 6, 100); // hal call reading imu data via i2c into buffer

		if (imu_accel_read_status != HAL_OK){ // check to make sure read went trhough properly
			fault_flags.imu_fault = true;
			return;
		}
		// bitwise shifting data byte into separate data pieces
		imu.accel_x = (int16_t)((buffer[0] << 8) | buffer[1]);
		imu.accel_y = (int16_t)((buffer[2] << 8) | buffer[3]);
		imu.accel_z = (int16_t)((buffer[4] << 8) | buffer[5]);
		// subtract real data from offset determined from init
...
void imu_init(void){
	...
	imu_who_am_i = 0;
	imu_wake_write_status = HAL_I2C_Mem_Write(&hi2c1, WRITE_ADDRESS, WAKE_REG, I2C_MEMADD_SIZE_8BIT, &sleep_bit, 1, 100);

	if (imu_wake_write_status != HAL_OK){
		fault_flags.imu_fault = true;
	}

	imu_who_read_status = HAL_I2C_Mem_Read(&hi2c1, WRITE_ADDRESS, 117, I2C_MEMADD_SIZE_8BIT, &imu_who_am_i, 1, 100);
	if (imu_who_read_status != HAL_OK){
		fault_flags.imu_fault = true;
	}
// imu handshake successful (should be 0x68 on configured settings)
